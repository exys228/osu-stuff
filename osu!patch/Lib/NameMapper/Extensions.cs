...
namespace osu_patch.Lib.NameMapper
{
	static class Extensions
	{
		...
		public static List<MethodDef> GetUniqueMethods(this IList<MethodDef> methods) => methods.GroupBy(m => m.Body?.Instructions.Select(i => i.OpCode.Code)).Where(g => g.Count() == 1).Select(g => g.FirstOrDefault()).ToList();
